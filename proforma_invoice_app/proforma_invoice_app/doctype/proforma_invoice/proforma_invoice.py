...
# proforma_invoice_app/proforma_invoice_app/doctype/proforma_invoice/proforma_invoice.py
import frappe
...
#         naming_series: DF.Literal["PFM-INV-.YYYY.-"]
...
#         # This hook runs every time you save, even for drafts.
#         self.check_sales_order_quantity()
#     def validate(self):
#         super().validate()
...
#         )
#         self.check_qty_against_sales_order()
...
#                     WHERE sales_order_item=%s AND parent != %s
#                 """, (item.sales_order_item, self.name))[0][0] or 0

#                 if used_qty + item.qty > so_qty:
#                     frappe.throw(
#                         frappe._("Row #{0}: Qty exceeds Sales Order qty ({1}) for {2}")
#                         .format(item.idx, so_qty, item.item_code)
#                     )


#             self.check_sales_order_quantity()
#             # Corrected validation to check quantity against the linked Sales Order
#             self.validate_quantity_with_sales_order()
...
#     def check_qty_against_sales_order(doc):
#         for item in doc.items:
#             if item.sales_order_item:  # use the correct fieldname from child table
...
#                 if so_qty is None:
#                     frappe.throw(
#                         _("Row #{0}: Linked Sales Order Item not found for {1}").format(
#                             item.idx, item.item_code
#                         )
#                     )

#                 # Cumulative qty already used in other Proforma Invoices
#                 used_qty = frappe.db.sql("""
#                     SELECT SUM(pi_item.qty)
#                     FROM `tabProforma Invoice Item` pi_item
#                     JOIN `tabProforma Invoice` pi ON pi.name = pi_item.parent
#                     WHERE pi.docstatus < 2
#                     AND pi_item.sales_order_item = %s
#                     AND pi.name != %s
#                 """, (item.sales_order_item, doc.name))[0][0] or 0

#                 if used_qty + item.qty > so_qty:
#                     frappe.throw(
#                         _("Row #{0}: Total Proforma Invoice qty for {1} ({2}) exceeds Sales Order qty ({3})")
#                         .format(item.idx, item.item_code, used_qty + item.qty, so_qty)
#                     )
#     def check_sales_order_quantity(self):
#         for item in self.get("items"):
#             if item.prevdoc_docname and item.prevdoc_detail_docname:
#                 so = frappe.get_doc("Sales Order", item.prevdoc_docname)
#                 so_item_list = so.get("items", {"name": item.prevdoc_detail_docname})

#                 if so_item_list:
#                     so_item = so_item_list[0]
#                     # Calculate remaining quantity correctly
#                     remaining_qty = so_item.qty - flt(so_item.delivered_qty) - flt(so_item.invoiced_qty)

#                     if flt(item.qty) > flt(remaining_qty):
#                         frappe.throw(
#                             _("Row #{0}: Quantity {1} cannot be greater than the remaining quantity {2} in Sales Order {3}").format(
#                                 item.idx, item.qty, remaining_qty, so.name
#                             )
#                         )
#     def validate_quantity_with_sales_order(self):
#         for item in self.get("items"):
#             if item.prevdoc_docname and item.prevdoc_detail_docname:
#                 so = frappe.get_doc("Sales Order", item.prevdoc_docname)
#                 so_item = so.get("items", {"name": item.prevdoc_detail_docname})

#                 if so_item:
#                     remaining_qty = so_item[0].qty - (so_item[0].delivered_qty or 0) - (so_item[0].invoiced_qty or 0)
#                     frappe.msgprint(f"Validating item {item.item_code}: PI qty={item.qty}, remaining qty={remaining_qty}")
...
#         """
#         # if not frappe.db.get_single_value("Selling Settings", "allow_zero_qty_in_proforma_invoice"):
#         #     return
...
#                     frappe.msgprint(
#                         _(
#                             "Warning: Proforma Invoice {0} already exists against Customer's Purchase Order {1}"
...
#                     frappe.throw(
#                         _(
#                             "Proforma Invoice {0} already exists against Customer's Purchase Order {1}. To allow multiple Proforma Invoices, Enable {2} in {3}"
#                         ).format(
#                             frappe.bold(so[0][0]),
#                             frappe.bold(self.po_no),
#                             frappe.bold(
#                                 _("'Allow Multiple Proforma Invoices Against a Customer's Purchase Order'")
#                             ),
...
#                         frappe.msgprint(
#                             _("Expected Delivery Date should be after Proforma Invoice Date"),
#                             indicator="orange",
...
#         # if bypass credit limit check is set to true (1) at Proforma Invoice level,
...
#             where t1.name = t2.parent and t2.proforma_invoice = %s and t1.docstatus = 0""",
#             self.name,
#         )
...
#             frappe.throw(
#                 _("Sales Invoice {0} must be deleted before cancelling this Proforma Invoice").format(
#                     ", ".join(linked_invoices)
#                 )
#             )

#     def check_modified_date(self):
#         mod_db = frappe.db.get_value("Proforma Invoice", self.name, "modified")
...
#         # Upon Proforma Invoice Re-open, check for credit limit.
...
#                     "Proforma Invoice Item", {"parent": self.name, "item_code": item.item_code}, "supplier"
#                 )
...
#                     where poi.proforma_invoice_item = %s
#                         and poi.item_code = %s
...
#         """Returns True if there is any unreserved item in the Proforma Invoice."""

#         reserved_qty_details = get_sre_reserved_qty_details_for_voucher("Proforma Invoice", self.name)
...
#         notify=True,
#     ) -> None:
#         """Creates Stock Reservation Entries for Proforma Invoice Items."""
...
#         """Cancel Stock Reservation Entries for Proforma Invoice Items."""
...
#     """Returns the unreserved quantity for the Proforma Invoice Item."""
...
# @frappe.whitelist()
# def close_or_unclose_proforma_invoices(names, status):
#     if not frappe.has_permission("Proforma Invoice", "write"):
...
# def get_requested_item_qty(proforma_invoice):
#     result = {}
#     for d in frappe.db.get_all(
#         "Material Request Item",
#         filters={"docstatus": 1, "proforma_invoice": proforma_invoice},
#         fields=["proforma_invoice_item", "sum(qty) as qty", "sum(received_qty) as received_qty"],
#         group_by="proforma_invoice_item",
#     ):
#         result[d.proforma_invoice_item] = frappe._dict({"qty": d.qty, "received_qty": d.received_qty})
...
#         source_name,
#         {
#             "Proforma Invoice": {"doctype": "Material Request", "validation": {"docstatus": ["=", 1]}},
#             "Packed Item": {
#                 "doctype": "Material Request Item",
#                 "field_map": {"parent": "proforma_invoice", "uom": "stock_uom"},
...
#         sre_details = get_sre_reserved_qty_details_for_voucher("Proforma Invoice", source_name)

#     mapper = {
#         "Proforma Invoice": {"doctype": "Delivery Note", "validation": {"docstatus": ["=", 1]}},
#         "Sales Taxes and Charges": {"doctype": "Sales Taxes and Charges", "reset_value": True},
#         "Sales Team": {"doctype": "Sales Team", "add_if_empty": True},
#     }

#     # 0 qty is accepted, as the qty is uncertain for some items
#     has_unit_price_items = frappe.db.get_value("Proforma Invoice", source_name, "has_unit_price_items")
...
#     so = frappe.get_doc("Proforma Invoice", source_name)
#     target_doc = get_mapped_doc("Proforma Invoice", so.name, mapper, target_doc)

#     if not kwargs.skip_item_mapping and kwargs.for_reserved_stock:
#         sre_list = get_sre_details_for_voucher("Proforma Invoice", source_name)
...
#         where t2.parent=t1.name and t2.proforma_invoice=%s and t1.docstatus=1""",
#         source_name,
#     )
...
#             source_name,
#             {
#                 "Proforma Invoice": {"doctype": "Maintenance Schedule", "validation": {"docstatus": ["=", 1]}},
#                 "Proforma Invoice Item": {
#                     "doctype": "Maintenance Schedule Item",
#                     "field_map": {"parent": "proforma_invoice"},
#                 },
#             },
#             target_doc,
#         )
...
#             source_name,
#             {
#                 "Proforma Invoice": {"doctype": "Maintenance Visit", "validation": {"docstatus": ["=", 1]}},
#                 "Proforma Invoice Item": {
...
#     conditions = get_event_conditions("Proforma Invoice", filters)

#     data = frappe.db.sql(
#         f"""
#         select
#             distinct `tabProforma Invoice`.name, `tabProforma Invoice`.customer_name, `tabProforma Invoice`.status,
#             `tabProforma Invoice`.delivery_status, `tabProforma Invoice`.billing_status,
#             `tabProforma Invoice Item`.delivery_date
#         from
#             `tabProforma Invoice`, `tabProforma Invoice Item`
#         where `tabProforma Invoice`.name = `tabProforma Invoice Item`.parent
#             and `tabProforma Invoice`.skip_delivery_note = 0
#             and (ifnull(`tabProforma Invoice Item`.delivery_date, '0000-00-00')!= '0000-00-00') \
#             and (`tabProforma Invoice Item`.delivery_date between %(start)s and %(end)s)
#             and `tabProforma Invoice`.docstatus < 2
#             {conditions}
#         """,
#         {"start": start, "end": end},
#         as_dict=True,
...
#                     "doctype": "Purchase Order Item",
#                     "field_map": [
#                         ["name", "proforma_invoice_packed_item"],
#                         ["parent", "proforma_invoice"],
#                         ["uom", "uom"],
#                         ["conversion_factor", "conversion_factor"],
...
#     # po = frappe.get_list("Purchase Order", filters={"proforma_invoice":source_name, "supplier":supplier, "docstatus": ("<", "2")})
#     doc = get_mapped_doc(
#         "Proforma Invoice",
#         source_name,
#         {
...
# def set_delivery_date(items, proforma_invoice):
#     delivery_dates = frappe.get_all(
#         "Proforma Invoice Item", filters={"parent": proforma_invoice}, fields=["delivery_date", "item_code"]
#     )
...
# @frappe.whitelist()
# def make_work_orders(items, proforma_invoice, company, project=None):
#     """Make Work Orders against the given Proforma Invoice for the given `items`"""
...
#                 proforma_invoice_item=i["proforma_invoice_item"],
#                 project=project,
...
# @frappe.whitelist()
# def make_raw_material_request(items, company, proforma_invoice, project=None):
#     if not frappe.has_permission("Proforma Invoice", "write"):
#         frappe.throw(_("Not permitted"), frappe.PermissionError)
...
#         item["include_raw_materials_from_proforma_invoice"] = items.get("include_raw_materials_from_proforma_invoice")

#     items.update({"company": company, "proforma_invoice": proforma_invoice})
...
#     return make_inter_company_transaction("Proforma Invoice", source_name, target_doc)
...
#     def validate_proforma_invoice():
#         so = frappe.get_doc("Proforma Invoice", source_name)
#         for item in so.items:
#             if item.stock_reserved_qty > 0:
#                 frappe.throw(
#                     _(
#                         "Cannot create a pick list for Proforma Invoice {0} because it has reserved stock. Please unreserve the stock in order to create a pick list."
...
#     # Don't allow a Pick List to be created against a Proforma Invoice that has reserved stock.
...
#                 "field_map": {"parent": "proforma_invoice", "name": "proforma_invoice_item"},
...
#                 "field_map": {
#                     "parent": "proforma_invoice",
#                     "parent_detail_docname": "proforma_invoice_item",
...
# def update_produced_qty_in_so_item(proforma_invoice, proforma_invoice_item):
#     # for multiple work orders against same Proforma Invoice item
#     linked_wo_with_so_item = frappe.db.get_all(
#         "Work Order",
#         ["produced_qty"],
#         {"proforma_invoice_item": proforma_invoice_item, "proforma_invoice": proforma_invoice, "docstatus": 1},
#     )
...
#     frappe.db.set_value("Proforma Invoice Item", proforma_invoice_item, "produced_qty", total_produced_qty)


# @frappe.whitelist()
# def get_work_order_items(proforma_invoice, for_raw_material_request=0):
#     """Returns items with BOM that already do not have a linked work order"""
#     if proforma_invoice:
#         so = frappe.get_doc("Proforma Invoice", proforma_invoice)
...
#                             & (wo.proforma_invoice == so.name)
#                             & (wo.proforma_invoice_item == i.name)
#                             & (wo.docstatus.lt(2))
#                         )
#                         .run()[0][0]
#                     )
