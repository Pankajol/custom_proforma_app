...
erpnext.accounts.taxes.setup_tax_validations("Proforma Invoice");
...
frappe.ui.form.on("Proforma Invoice", {
    ...
    refresh: function (frm) {
            ...
            if (
                ...
                frm.add_custom_button(__("Update Items"), () => {
                    erpnext.utils.update_child_items({
                        frm: frm,
                        child_docname: "items",
                        child_doctype: "Proforma Invoice Detail",
        ...
        // Hide `Reserve Stock` field description in submitted or cancelled Proforma Invoice.
        if (frm.doc.docstatus > 0) {
        ...
        frm.add_custom_button(
            ...
            () => {
                erpnext.utils.map_current_doc({
                    method: "erpnext.buying.doctype.purchase_order.purchase_order.make_inter_company_proforma_invoice",
    ...
    onload: function (frm) {
        ...
        // On cancel and amending a Proforma Invoice with advance payment, reset advance paid amount
        if (frm.is_new()) {
        ...
        const dialog = new frappe.ui.Dialog({
            ...
            fields: [
                ...
                { fieldtype: "Column Break" },
                {
                    ...
                    onchange: () => {
                        let proforma_invoice_item = dialog.get_value("add_item");

                        if (proforma_invoice_item) {
                            frm.doc.items.forEach((item) => {
                                if (item.name === proforma_invoice_item) {
                                    let unreserved_qty =
                                        (flt(item.stock_qty) -
                                    ...
                                    if (unreserved_qty > 0) {
                                        dialog.fields_dict.items.df.data.forEach((row) => {
                                            if (row.proforma_invoice_item === proforma_invoice_item) {
                                    ...
                                    dialog.fields_dict.items.df.data.push({
                                        proforma_invoice_item: item.name,
                ...
                { fieldtype: "Section Break" },
                {
                    ...
                    fields: [
                        {
                            fieldname: "proforma_invoice_item",
                            fieldtype: "Link",
                            label: __("Proforma Invoice Item"),
                            options: "Proforma Invoice Item",
                            reqd: 1,
                            in_list_view: 1,
                            get_query: () => {
                            ...
                            onchange: (event) => {
                                if (event) {
                                    ...
                                    frm.doc.items.forEach((item) => {
                                        if (item.name === item_row.proforma_invoice_item) {
...
frappe.ui.form.on("Proforma Invoice Item", {
    ...
    qty: function(frm, cdt, cdn) {
        var row = locals[cdt][cdn];
        // Check if the item is linked to a Sales Order via `so_detail`
        if (row.so_detail) {
            frappe.db.get_value("Sales Order Item", row.so_detail, "qty", (r) => {
                if (r.message && row.qty > r.message.qty) {
                    frappe.msgprint({
                        title: __("Quantity Exceeded"),
                        indicator: "red",
                        message: __("Quantity cannot be greater than the original Sales Order quantity: {0}", [r.message.qty])
                    });
                    // This will prevent the form from being submitted on the client side
...
proforma_invoice_app.ProformaInvoiceController = class ProformaInvoiceController extends proforma_invoice_app.SellingController{
            ...
            if (this.frm.has_perm("submit")) {
                if (doc.status === "On Hold") {
                    ...
                    if (flt(doc.per_delivered) < 100 || flt(doc.per_billed) < 100) {
                        // close
                        this.frm.add_custom_button(__("Close"), () => this.close_proforma_invoice(), __("Status"));
                    }
                ...
                if (doc.status !== "On Hold") {
                    ...
                    if (this.frm.has_perm("submit")) {
                        if (flt(doc.per_delivered) < 100 || flt(doc.per_billed) < 100) {
                            ...
                            this.frm.add_custom_button(
                                __("Hold"),
                                () => this.hold_proforma_invoice(),
                                __("Status")
                            );
                            // close
                            this.frm.add_custom_button(
                                __("Close"),
                                () => this.close_proforma_invoice(),
                                __("Status")
                            );
                        }
                    }
                        ...
                        primary_action: function () {
                            ...
                            me.frm.call({
                                ...
                                args: {
                                    items: data,
                                    company: me.frm.doc.company,
                                    proforma_invoice: me.frm.docname,
    ...
    make_purchase_order() {
        ...
        if (!pending_items) {
            frappe.throw({
                message: __("Purchase Order already created for all Proforma Invoice items"),
                title: __("Note"),
            });
        }
                    ...
                    callback: function (r) {
                        if (!r.exc) {
                            if (!args.against_default_supplier) {
                                ...
                                frappe.route_options = {
                                    proforma_invoice: me.frm.doc.name,
                                };
